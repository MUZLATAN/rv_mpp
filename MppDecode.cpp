...
int whale::vision::MppDecode::initMpp() {
	if (m_bInit) {
		std::cout << "initMpp : " << m_bInit<<std::endl;
		return 0;
	}
    deInitMpp();
...
int whale::vision::MppDecode::initMppMtx()
{
	// m_QueMtx.lock();
	initMpp();
	return 0;
...
void whale::vision::MppDecode::deInitMpp() {
	...
	m_bInit = false;
	printf("deInitMpp 11111111111111111111111111111111111111111111111111111111111111\n");
}

int whale::vision::MppDecode::decode(AVPacket *av_packet, cv::Mat &image, const std::string &p_sn) {
	RK_U32 pkt_done = 0;
	RK_U32 pkt_eos = 0;
	RK_U32 err_info = 0;
	MPP_RET ret = MPP_OK;
	...
	if(MPP_OK != ret)
	{
		std::cout  << "mpp_packet_init error ... : " << ret<<std::endl;
		return ret;
	}
	std::vector<std::string>::iterator iter = std::find(m_strSnVec.begin(), m_strSnVec.end(), p_sn);
	if (iter != m_strSnVec.end()) {
		int nIndex = std::distance(m_strSnVec.begin(), iter);
		av_packet->pts = nIndex;//ans即为5在数组中的序号
	}
	...
	do {
		...
		if (!pkt_done) {
			...
			if (MPP_OK == ret) {
				...
				nPutErrNum++;
				// std::cout << " decode put package error....";
			}
		}
		...
		try_again:
			...
			if (frame) {
				if (mpp_frame_get_info_change(frame)) {
					...
					frame_count++;
//					std::cout<< "-------decode get frame get frame: " << frame_count<<std::endl;
					if (!err_info) {
						YUV420SP2Mat(image);
						if (!image.empty()) {
							int nIndex = mpp_frame_get_pts(frame);
							if (nIndex >= 0 && nIndex < m_strSnVec.size()){
								cb_ = m_callBackMap[m_strSnVec[nIndex]];
								cb_(image);
							}
						}
					}
...
int whale::vision::MppDecode::decodeMtx(AVPacket* av_packet, cv::Mat& image, const std::string &p_sn)
{
	// m_QueMtx.lock();
